...
- **全文模式（Full-Text Mode）**：向量檢索是「篩選式」的，遇到問法太抽象、或關鍵內容藏在冷門角落時，有可能篩選失準、答非所問。提問輸入框上方有「全文模式」開關，開啟後會跳過向量篩選，把當下所有已載入的指引文件（含圖片）完整送給 AI，保證不遺漏任何內容，但 token 用量與費用會明顯提高，畫面會即時顯示預估花費供你評估後再送出。
